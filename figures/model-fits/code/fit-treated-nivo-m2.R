...
dev <- function( x, N=210 ){
	...
	ipd_fake <- data.frame(
		time=get_s_M2( N, mean=x[1], sd=x[2], lower_growth=x[3] ), 
		status=1, treat=2
	)
	...
	if( p < p_min ){
		...
		if( Sys.getenv("DRAFT_MODE") != "" ){
			saveRDS( x_min, file="data/M2-nivo-xmin-draft.Rds" )
		} else {
			saveRDS( x_min, file="data/M2-nivo-xmin.Rds" )
		}
	}
	p
}

th_init <- c(-4.1994636, 0.6802203, 0.7546553, 150)
th_prior <- function() c(runif(1,-6,-1), runif(1,0.5,1.5), runif(1,0.2,1), runif(1,1,1000))
th_mut <- c(.1,.1,.05, 5)
p_min <- Inf
...
if( Sys.getenv("DRAFT_MODE") != "" ){
	...
	saveRDS( r, file="data/M2-nivo-abc-result-draft.Rds" )
	R.utils::touchFile("data/M2-nivo-abc-result.Rds") 
} else {
	r <- abc( th_init, th_mut, N=500, dev, th_prior )
	saveRDS( r, file="data/M2-nivo-abc-result.Rds" )
}
