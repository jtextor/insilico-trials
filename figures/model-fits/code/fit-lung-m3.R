...
dev <- function( x, N=228 ){
	...
	ipd_fake <- data.frame(time=get_s_M3(N, mean=x[1], sd=x[2], lower_growth=x[3]), status=2, treat=2)
	...
	if( p < p_min ){
		...
		if( Sys.getenv("DRAFT_MODE") != "" ){
			saveRDS( x_min, file="data/M3-lung-xmin-draft.Rds" )
		} else {
			saveRDS( x_min, file="data/M3-lung-xmin.Rds" )
		}
	}
	p
}
...
th_init <- c(-2,1,0.3)
th_prior <- function() c(runif(1,-6,0), runif(1,0.5,1.5), runif(1,0,1))
th_mut <- c(.1,.1,.05)
...
if( Sys.getenv("DRAFT_MODE") != "" ){
	...
	saveRDS( r, file="data/M3-lung-abc-result-draft.Rds" )
	R.utils::touchFile("data/M3-lung-abc-result.Rds") 
} else {
	r <- abc( th_init, th_mut, N=500, dev, th_prior )
	saveRDS( r, file="data/M3-lung-abc-result.Rds" )
}
