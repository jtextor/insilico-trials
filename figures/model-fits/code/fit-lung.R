...
dev <- function( x, N=228 ){
	...
	ipd_fake <- data.frame(time=get_s_M1(N, mean=x[1], sd=x[2], lower_growth=x[3]), status=2, treat=2)
	...
	if( p < p_min ){
		...
		if( Sys.getenv("DRAFT_MODE") != "" ){
			saveRDS( x_min, file="data/M1-lung-xmin-draft.Rds" )
		} else {
			saveRDS( x_min, file="data/M1-lung-xmin.Rds" )
		}
	}
	p
}

th_init <- c(2.03022245518433, 0.775418948419806, 0.903180310449139)
th_prior <- function() c(runif(1,1,3), runif(1,0.5,1.5), runif(1,0.3,1))
th_mut <- c(.1,.1,.05)
...
if( Sys.getenv("DRAFT_MODE") != "" ){
	...
	saveRDS( r, file="data/M1-lung-abc-result-draft.Rds" )
	R.utils::touchFile("data/M1-lung-abc-result.Rds") 
} else {
	r <- abc( th_init, th_mut, N=500, dev, th_prior )
	saveRDS( r, file="data/M1-lung-abc-result.Rds" )
}
