...
dev <- function( x, N=250 ){
	...
	if( p < p_min ){
		...
		if( Sys.getenv("DRAFT_MODE") != "" ){
			saveRDS( x_min, file="data/M1-ipi-xmin-draft.Rds" )
		} else {
			saveRDS( x_min, file="data/M1-ipi-xmin.Rds" )
		}
	}
	p
}
...
th_prior <- function() c(runif(1,1.5,3.5), runif(1,0.5,1.5), runif(1,0.2,1), runif(1,10,15))
th_mut <- c(.1,.1,.05,.5)
p_min <- Inf
...
if( Sys.getenv("DRAFT_MODE") != "" ){
	...
	saveRDS( r, file="data/M1-ipi-abc-result.Rds" )
}
