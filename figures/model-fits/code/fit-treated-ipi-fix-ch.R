...
dev <- function( x, N=250 ){
	...
	if( p < p_min ){
		...
		if( Sys.getenv("DRAFT_MODE") != "" ){
			saveRDS( x_min, file="data/M1-ipi-fix-ch-xmin-draft.Rds" )
		} else {
			saveRDS( x_min, file="data/M1-ipi-fix-ch-xmin.Rds" )
		}
	}
	p
}

th_init <- c(2, 1, 0.95, 10)
th_prior <- function() c(runif(1,1.5,3.5), runif(1,0.5,1.5), runif(1,0.6,0.6), runif(1,10,15))
th_mut <- c(.1,.1,0,.5)
p_min <- Inf
...
if( Sys.getenv("DRAFT_MODE") != "" ){
	...
	saveRDS( r, file="data/M1-ipi-fix-ch-abc-result-draft.Rds" )
	touch("data/M1-ipi-abc-fix-ch-abc-result.Rds") 
} else {
	r <- abc( th_init, th_mut, N=500, dev, th_prior )
	saveRDS( r, file="data/M1-ipi-fix-ch-abc-result.Rds" )
}
